...
export default function PageClient({
  ...
  // 🚀 Determine the copy based on the Edge variant
  const supportTitle =
    supportVariant === "test"
      ? "Buy me a Coffee ☕"
      : profileData.support?.title || "Support the Stream";

  const supportSubtitle =
    supportVariant === "test"
      ? "Fuel the late-night coding sessions"
      : profileData.support?.subtitle || "Drop a tip on Creators.sa";
    ...
    <SpotlightBackground>
      <main className="relative flex h-dvh w-full flex-col items-center justify-center overflow-hidden px-4 font-sans sm:px-6">
        ...
        <div className="z-10 flex w-full max-w-lg flex-col gap-4">
          ...
          >
            {/* 🚀 Render the Support Card with the passed data */}
            {profileData.support && (
              <SupportCard
                title={supportTitle}
